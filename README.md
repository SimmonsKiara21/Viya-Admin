...
- **Edit** on an open talent file changes name, nickname, age, email, phone, start date, class time, their enrollment tag, and **Overdue since**. Status and overdue since are also on the open file — Pending to Current after a deposit, or correct the first missed date when Square is wrong.
- **Subscriptions** has three Square plans: standard ($51.49 with tax), OG ($4.99), and the $100 plan (Scarlett Petroff). All three share the same Square description. On a talent file, the **Subscription** tab sets status (Active, Interested, Paused) and which of those three plans they are on.
...
- **Photoshoots** is named by you, not only by month. Add the next shoot with the title and notes you want (looks, wardrobe, who should be there). Rename any month or shoot, duplicate a list (people included), or delete one. Headshots, Refresh, and Received stack together; Scheduled and Full sit beside them. Prior shoots sit under Prior. On a talent file, the **Photoshoot** tab has measurements (height, bust, waist, hips, dress, shoe) plus that person’s shoot notes.
