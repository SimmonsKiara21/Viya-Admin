...
- **Talent** is the academy roster (modeling and acting included). Subscribers are not counted here — they are on **Subscriptions**. Filter by program or status. Sort **A–Z**, **Z–A**, newest start, or oldest start. Home **Pending** opens that filter sorted by start date.
