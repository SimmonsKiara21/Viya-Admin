...
- **Talent** is the academy roster (modeling and acting included). Subscribers are not counted here — they are on **Subscriptions**. Filter by program, **PIF / payment plan**, or status. Sort **A–Z**, **Z–A**, newest start, or oldest start. Home **Pending** opens that filter sorted by start date. **PIF** is the lump-sum plan. Green **Paid in full** is only after academy tuition is actually paid — a pending PIF deposit does not get that highlight.
